...
sources: $(SPECFILE)
	...
	tar --owner=0 --group=0 -rf $(TARNAME).tar $(TARNAME)/$(SPECFILE) $(TARNAME)/PKG-INFO $(TARNAME)/openstack-nova-*.service $(TARNAME)/nova-dist.conf $(TARNAME)/nova-ifc-template $(TARNAME)/nova-polkit.pkla $(TARNAME)/nova-polkit.rules $(TARNAME)/nova-sudoers $(TARNAME)/nova.logrotate
